...
public class ApplicationStartupListener implements ServletContextListener, HttpSessionListener, HttpSessionAttributeListener {
    ...
    @Inject
    private OscarReviewRepository _employeeRepository;
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        if (_employeeRepository.count() == 0) {
            try {
               _employeeRepository.create(new OscarReview("actor", "Lee Jung-jae", "Squid game!!", "Youngjae Lee"));
               _employeeRepository.create(new OscarReview("actress", "HoYeon Jung", "Squid game!!", "Youngjae Lee"));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
